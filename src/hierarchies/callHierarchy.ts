...
export class CallHierarchyProvider extends Hierarchy<CallHierarchyNode> {
  ...
  constructor(languageClient: LanguageClient, qualified: boolean) {
    ...
    this._dispose.push(
      commands.registerCommand('ccls.call.useCallers', () => this.updateCallee(false))
    );
    this._dispose.push(
      commands.registerCommand('ccls.call.useCallees', () => this.updateCallee(true))
    );
  }

  public onTreeItem(ti: TreeItem, element: CallHierarchyNode) {
    if (element.callType === CallType.Base) ti.iconPath = this.baseIcon;
    else if (element.callType === CallType.Derived) ti.iconPath = this.derivedIcon;
  }
